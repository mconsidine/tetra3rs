...
- **Same verification threshold as LIS.** Tracking doesn't relax the statistical false-positive test (`match_threshold`, a per-solve false-positive budget), so extremely sparse fields (where only 2–3 catalog stars are visible) can still fail verification even though Wahba's problem is solvable. Tracking does have one statistical edge over LIS: because the hint is independent of the centroids, none of the matched stars are discounted as hypothesis stars, so the same number of matches carries more evidence. Raising `match_threshold` accepts sparser fields at an explicitly chosen false-positive risk.
